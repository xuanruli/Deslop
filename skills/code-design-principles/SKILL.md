...
为"赶快跑通"在已有的烂 / 临时结构上继续叠加，是最常见的偷懒。新增前先判断当前结构扛不扛得住这次改动：

- 扛得住 → 顺着写。
- 扛不住 → 先做个小重构再在干净基础上加，别 pile on workaround / 特例 / 复制粘贴。

判断信号：如果"让它跑通"的代价是又加一层绕路，多半该先重构。
权衡：重构范围大或涉及业务取舍时，先跟用户说"建议先重构 X 再做"让其拍板，而不是默默选最省事那条路。
...
## 17.1 注释与 docstring 保持极简

- docstring 默认一行，确有必要才多行。
- 注释默认不写，只在别人可能误解逻辑时补一行。
- 禁止版本更替类注释（"原来是 X 改成 Y"）—— 版本变化交给 git。

## 17.2 不留 AI slop，与所在文件风格一致

写完 / review diff 时，删掉本次引入的、人类不会写、和周围格格不入的痕迹：

- **类型逃逸**：禁止为绕过类型报错而 `as any` / `# type: ignore` / `@ts-ignore`，要真正修类型。
- **多余防御**：别在可信 / 已校验的调用链上加冗余 try/catch 或防御检查（和 #8、#13 互补：该防的防，不该防的别瞎防）。
- **风格不一致**：新代码的命名、注释密度、写法要匹配所在文件的既有风格，不要鹤立鸡群。

判断标准就一句："**和这个文件的既有风格一致吗**"——slop 的本质是和周围不一致的 AI 痕迹。
...
- 为赶快跑通在烂结构上堆 workaround / 特例，而非该重构时提出重构
...
- 默认参数是 `[]` / `{}` / `set()`
- 注释只是把代码翻译成英文
- 多行 docstring 但其实一行就够
- 版本更替类注释（"原来是 X 现在改成 Y"）写进了代码而非 git
- `as any` / `type: ignore` / `@ts-ignore` 绕过类型报错而非真正修
- 在可信 / 已校验调用链上加冗余 try/catch 或防御检查
- 新代码风格（命名 / 注释密度）与所在文件不一致
- URL / 超时 / 神秘数字硬编码
